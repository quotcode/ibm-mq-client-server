...
public class ProcessCSVResponse {
    ...
    public void parseCSVResponse(MqStage mqStageRsp) throws SQLException, IOException {
        ...
        List<List<String>> recordsList = recordStrList.stream().map(ele -> Arrays.stream(ele.split(",")).toList()).toList();
        ...
        // 1st list contains the header column. Also, we are ignoring 1st value which is record-type as we only need the nodes that we can map with metadata file nodes
        ...
        if (metadataDao.findAllRecordTypes().contains(rspRecordType)) {
            ...
            if (fileNodes.stream().collect(Collectors.toSet()).equals(headerColumn.stream().collect(Collectors.toSet()))) {
                ...
                for(Map<String,String> map: recordsMapList){
                    ...
                    sb.append(map.keySet())
                            .append(" ) values ( ")
                            .append(map.values().stream().map(ele -> "'"+ele+"'").toList())
                            .append(" )");
